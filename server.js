...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==='GET'&&url.pathname==='/leaderboard'){
    const game=String(url.searchParams.get('game')||'actual');if(!games.has(game))return json(res,400,{error:'Juego inválido'});return json(res,200,{generation:26,storage:'temporal-memory',game,scores:top(game)});
  }
